...
export async function updatePerk(req, res, next) {
  try {
    // validate only the fields provided in req.body
    const { value, error } = perkSchema.validate(req.body, { 
      abortEarly: false,
      allowUnknown: false,
      stripUnknown: true,
...
// PUT → full update (expects all required fields)
export async function updatePerk(req, res, next) {
  try {
    const { value, error } = perkSchema.validate(req.body, {
      abortEarly: false,
...
// PATCH → partial update (only updates the provided fields)
// Partial update schema (no defaults, all optional)
const patchPerkSchema = Joi.object({
...
export async function patchPerk(req, res, next) {
  try {
    // validate only provided fields
    const { value, error } = patchPerkSchema.validate(req.body, {
      abortEarly: false,
      allowUnknown: false,
      stripUnknown: true
    });
